...
dirs := $(BUILD)/obj $(BUILD)/bin $(BUILD)/lib $(BUILD)/test $(BUILD)/obj/toollib/ap $(BUILD)/obj/toollib/csv $(BUILD)/obj/toollib/vec $(BUILD)/obj/toollib/assoc $(BUILD)/obj/toollib/pgm $(BUILD)/obj/trs/error $(BUILD)/obj/trs/horn $(BUILD)/obj/trs/trs.cg.nasm_x86 $(BUILD)/obj/trs/trs.cg.llvm $(BUILD)/obj/trs/trs.cg.lisp $(BUILD)/obj/trs/trsc

.PHONY: clean all test

all: $(dirs) $(BUILD)/lib/libap.a $(if $(RELEASE),$(BUILD)/lib/ap.so,) $(BUILD)/lib/libcsv.a $(if $(RELEASE),$(BUILD)/lib/csv.so,) $(BUILD)/lib/libvec.a $(if $(RELEASE),$(BUILD)/lib/vec.so,) $(BUILD)/lib/libassoc.a $(if $(RELEASE),$(BUILD)/lib/assoc.so,) $(BUILD)/lib/libpgm.a $(if $(RELEASE),$(BUILD)/lib/pgm.so,) $(BUILD)/lib/liberror.a $(if $(RELEASE),$(BUILD)/lib/error.so,) $(BUILD)/lib/libhorn.a $(if $(RELEASE),$(BUILD)/lib/horn.so,) $(BUILD)/lib/libtrs.cg.nasm_x86.a $(if $(RELEASE),$(BUILD)/lib/trs.cg.nasm_x86.so,) $(BUILD)/lib/libtrs.cg.llvm.a $(if $(RELEASE),$(BUILD)/lib/trs.cg.llvm.so,) $(BUILD)/lib/libtrs.cg.lisp.a $(if $(RELEASE),$(BUILD)/lib/trs.cg.lisp.so,) $(BUILD)/bin/trsc
...
bin = $(patsubst src/%,$(BUILD)/obj/%.o,$(call rwildcard,src/trs/error, *.c))
libbin += $(bin)
...
bin = $(patsubst src/%,$(BUILD)/obj/%.o,$(call rwildcard,src/trs/trs.cg.llvm, *.c))
libbin += $(bin)
$(BUILD)/lib/libtrs.cg.llvm.a: $(bin)
	$(AR) qc $@ $^

$(BUILD)/lib/trs.cg.llvm.so: $(bin)
	$(CXX) -o $@ $^ -std=gnu++17 -Wall -Wextra -Wpedantic --shared $(if $(DEBUG),-ggdb,)
