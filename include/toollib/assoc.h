...
#define assoc_GEN_FOR_TYPE(T)                                                                                                                       \
	...
	static inline T*             assoc_set_##T(assoc* a, const char* key)                 { return (T*) assoc_set(a, key, sizeof(T)); }             \
	...
	static inline T*             assoc_get_##T(assoc a, const char* key)                  { return (T*) assoc_get(a, key); }                        \
	static inline T              assoc_getOrDefault_##T(assoc a, const char* key, T def)  { T* p = (T*) assoc_get(a, key); return p?*p:def; }       \
	...
	static inline T*             assoc_valueAt_##T(assoc a, size_t index)                 { return (T*) assoc_valueAt(a, index); }                  \
	static inline T              assoc_valueAtOrDefault_##T(assoc a, size_t index, T def) { T* p = (T*) assoc_valueAt(a, index); return p?*p:def; }
